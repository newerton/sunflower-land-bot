...
""".format(localVersion[0], localVersion[1], gitHubVersion[0], gitHubVersion[1])
    ...
    def gitHubVersion(self):
        self.importLibs()
        data = requests.get(
            url='https://raw.githubusercontent.com/newerton/sunflower-land-bot/main/config/version.yaml', timeout=2)
        try:
        ...
        if versionLocalApp >= versionGithubApp:
            ...
            if len(diff) > 0 and 'dictionary_item_added' in diff:
                ...
                try:
                    ...
                    print(
                        Fore.WHITE + '\nSee: https://github.com/newerton/sunflower-land-bot/blob/main/config/EXAMPLE-config.yaml' + Fore.RESET)
                    print(
                        Fore.GREEN + '\n*********************************************************************************' + Fore.RESET)
